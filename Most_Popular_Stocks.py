...
# get bloomberg page
original = requests.get('https://www.webull.com/quote/us/hots')
original
original.text
...
for names in stock_names:    #  to loop through repo underscore names and just print each of the items that exists within that list
    ...
    ## 2nd dataset 
Div_Yield = soup.find_all('div', class_='table-cell')

Div_Yields = []

for item in Div_Yield:         
    name = item.text
    #print('Part 1: Raw: ', name)
    ...
    name = name.replace('\n','')    # push to single line
    #print('Part 3: Updated: ', name)
    Div_Yields.append(name)
len(Div_Yields)

for names in Div_Yields: 
    print(names)
...
# create dictionary
dictionary = {'Most Popular Stocks': stock_names, 'Div_Yields': Div_Yields}

## put this together into a dataframe


a = {'Most Popular Stocks': stock_names, 'Div_Yields': Div_Yields}
df = pd.DataFrame.from_dict(a, orient='index')
df = df.transpose()
print(df.sample(10).to_markdown())
df.to_csv(r'C:\Users\premd\OneDrive\Desktop\HHA_507_\web-scraping\DATA_CSV_FILE\Most_Popular_Stocks.csv')